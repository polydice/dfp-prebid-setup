...
import custom_dfp.create_sponsorship_line_items
...
def setup_partner(user_email, advertiser_name, order_id, order_name, placements, ad_units, sizes, bidder_code, prices,
    ...
    logger.info(f"Creating line items {line_items_config[0]['name']}...")
    line_item_ids = custom_dfp.create_sponsorship_line_items.create_line_items(
        line_items_config)
...
def create_line_item_configs(prices, order_id, placement_ids, ad_unit_ids, bidder_code, sizes, hb_bidder_key_id,
    ...
    for price in prices:
        ...
        line_item_name = line_item_format.format(
            ...
            # Avoid line_item name confliction
            price=price_str[:-1]
        )
        ...
        config = custom_dfp.create_sponsorship_line_items.create_line_item_config(name=line_item_name, order_id=order_id,
...
def main(settings, dry_run=False):
    ...
    order_id = getattr(settings, 'DFP_ORDER_ID', None)
    if order_id is None:
        raise MissingSettingException('DFP_ORDER_ID')
    ...
    line_item_format = getattr(
        settings, 'DFP_LINE_ITEM_FORMAT', u'{bidder_code}: HB ${price}')
    ...
    # DEBUG - upload only 1 line-item
    # prices = prices[:1]
    prices_summary = get_prices_summary_string(
        prices, price_buckets['precision'])
    ...
    if not dry_run:
        setup_partner(user_email, advertiser_name, order_id, order_name, placements, ad_units,
                      sizes, bidder_code, prices, num_creatives, currency_code, line_item_format)
