...
def create_line_item_config(name, order_id, placement_ids, ad_unit_ids, cpm_micro_amount, sizes, hb_bidder_key_id,
    ...
    line_item_config = {
        ...
        # Workaround NOT_ENOUGH_INVENTORY error
        'allowOverbook': True,
        'skipInventoryCheck': True,
        # END Workaround NOT_ENOUGH_INVENTORY error
    ...
    if ad_unit_ids is not None:
        line_item_config['targeting']['inventoryTargeting']['targetedAdUnits'] = [{'adUnitId': id}
                                                                                  for id in ad_unit_ids]
